...
| D172 | **A basket of courses is N admissions, one per course — never one admission holding many courses.** | The operator asked to tick several courses on New Admission and quote one figure, and the question was where the plurality lives. It does not live in the row. `batches.course_id` is NOT NULL so a batch belongs to one course; `student_admissions.batch_id` is one scalar so the second batch assignment overwrites the first; `activationGaps()` asks only whether `batch_id` is null, so an admission seated in one of three courses passes the activation gate; `completed_on` is one date; a certificate is drafted from one enrolment; and `uq_sadm_live` is `(student_id, course_id, active_guard)`, so courses held in a pivot would not be guarded at all. The fee generator decides it without an opinion: `generation_key` is `structure:{admission_id}:{head}` under `uq_sf_generation`, so a second course fee on one admission is **skipped as a duplicate** and three course fees collapse into one blended charge. So `AdmissionService::createMany()` writes one admission per ticked course, **inside one transaction** — which is the part that had to be built rather than reused, because `create()` runs its duplicate guard outside its own transaction and a loop over it commits a partial basket while returning a 422 that reads as "nothing was saved". The basket's discount is stored per course pro-rata via `Money::allocate()` (largest remainder, so the shares add back exactly), and `admission_fee` / `registration_fee` are charged once by default because they are per-student heads in practice. **The consequence to know:** commission is resolved per admission, so a `fixed` rule or a `max_commission_amount` is applied N times, and — the silent mirror — N smaller releases can each fall under `collaborator.commission_min_entry_amount` and be skipped entirely. Not new (three admissions by hand always did this) but newly easy, and now written in the method's own docblock. Needed no migration, which is the test of the design. |
---
...
### 2026-09-26 — A basket of courses is N admissions, not one admission with N courses

"New admission ma Course ko multiply kar saka, or Agreed figures is hisab sa ho gya" — tick several
courses on one screen and have the quoted figure follow. Two designs were possible and the difference
is structural, so seven readers surveyed the dependency surface before anything was written.

**(B) one admission holding many courses was refused by the schema, not by taste.** `batches.course_id`
is NOT NULL, so a batch belongs to exactly one course; `student_admissions.batch_id` is one scalar, so
the second and third batch assignments would silently overwrite the first; `activationGaps()` asks only
`if ($admission->batch_id === null)`, so an admission with a seat in one of three courses would pass
the activation gate; `completed_on` is one date; a certificate is drafted from one enrolment; and
`uq_sadm_live` is `(student_id, course_id, active_guard)`, so courses living in a pivot would not be
guarded at all — a student live on A+B+C could be admitted again for B. The fee generator settles it
on its own: `generation_key` is `structure:{admission_id}:{head}` under `uq_sf_generation`, so a second
course fee for one admission is **skipped as a duplicate**, and three course fees would collapse into
one blended charge. So the screen totals; the database does not.

**(A) is what shipped: `AdmissionService::createMany()`, one admission per ticked course, in ONE
transaction.** That last clause is the substance. The obvious implementation — call `create()` in a
loop — is broken in a way that is hard to see and easy to ship: `create()` opens its own
`db->transaction()` and runs `assertNoLiveAdmission()` **outside** it. A basket whose third course was
already live would commit the first two, then throw. And because `CourseRuleException extends
ValidationException`, what the operator gets is a **422 redirect back to the form with a field error**
— which reads as *nothing was saved*. Two admissions with real numbers would be sitting there, and
the next attempt would refuse those two as well. `nothing_is_created_when_one_course_in_the_basket_is_already_live`
is the assertion that closes it.

Four more judgements, each of them a thing that would have been wrong by default:

- **Every course is checked before any row is written, and every conflict is reported together.**
  `assertNoLiveAdmission()` throws on the first clash; in a loop that names one course and hides the
  rest, so the operator fixes one, resubmits, and meets the next one. `assertNoLiveAdmissions()` is one
  query and one sentence naming all of them.
- **The money payload had to split in two.** `figuresFrom()` prefers an override over the course's own
  price, so the old flat `course_fee` posted against three courses would have priced **every course at
  the first one's fee** — silently, with a valid-looking admission for each. Per course now:
  `lines[<course_id>][course_fee|admission_fee|registration_fee|monthly_fee]`. Per basket: the discount,
  the scholarship and the reason.
- **The discount is agreed on the basket and stored per course, pro-rata**, through
  `Money::allocate()` with largest-remainder placement — so three equal lines and a 1,000 discount
  store 333.34 / 333.33 / 333.33 and not 999.99. A split proportional to gross also cannot push a line
  past its own `chk_sadm_discount_ceiling`, because a share of a whole that is within the total is
  within that line's part of it. The basket total is checked *before* the transaction so the refusal
  talks about the package rather than about a course the operator never typed into.
- **`admission_fee` and `registration_fee` are charged once for the basket by default.** They are
  ...
  Defaulting them from every course would have added 6,000 of invented charge to a three-course quote
  where every figure came from the catalogue and looked right. A visible, explained checkbox switches
  it to per-course; the operator chose "once" as the default.

**The money consequence that is written down rather than left to be discovered.** A partner commission
is resolved **per admission**: `document_key` is one per admission, and `promiseFor()` applies both a
`fixed` amount and `max_commission_amount` per entitlement. A basket of three earns a "PKR 5,000 per
enrolment" rule **three times**, and up to three times a cap. The mirror is worse because it is silent:
`CommissionCalculator` step C7 skips any release below `collaborator.commission_min_entry_amount`, so
splitting one sale into three smaller documents can make **all three** fall under the minimum and earn
nothing. Neither is new — creating three admissions by hand has always done this, and
`collaborator_commission_settings` has no course dimension so the rate cannot vary per course either.
What is new is that one click does it, from a screen that shows no commission at all. It is in the
`createMany()` docblock for the next person who reads the method.

**Two bugs in the form, both found by reasoning about the browser rather than by the screen looking
wrong.** A checkbox with a `name` posts in **DOM order** — here alphabetical by category — while the
server reads the order of `course_ids` to decide which line carries the one-off fees. The operator's
first tick and the server's first line would have been different courses, the preview would have
disagreed with what was stored, and nothing would have said so. The checkboxes now carry no name and
hidden inputs rendered from `picked` are what posts. And `:value` bound through `Number()` rewrites a
field mid-edit: typing `1500.` normalises to `1500` and deletes the dot just pressed. Display reads the
raw keystrokes; only the arithmetic coerces.

**No migration.** Not a happy accident — it is the test of the design. A change to how admissions are
sold that needs no new column is a change that did not fight the schema.

The category name on the picker comes back by `leftJoin` on the query that was already being run, because
`admin.admissions.create` carries a measured `query_budget` of 20 and PRF-01 reads it as a ceiling. An
eager load would have been a second query for a label.

Validation moved out of the controller into `StoreAdmissionRequest` — one row off **T48**.
