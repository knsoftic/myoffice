...
@section('header')
    <x-ui.page-header :title="'Receipt ' . $payment->receipt_no" subtitle="Printable copy" icon="printer">
        <x-slot:actions>
            <x-ui.button variant="ghost" :href="route('admin.fee-payments.show', $payment)" icon="arrow-left">Back</x-ui.button>
    ...
    <div class="mx-auto max-w-2xl rounded-xl border border-slate-200 bg-white p-8 print:border-0 print:p-0 dark:border-slate-700 dark:bg-slate-900">
        <div class="flex items-start justify-between border-b border-slate-200 pb-4 dark:border-slate-700">
            <div>
                ...
                <p class="text-sm text-slate-500 dark:text-slate-400">Fee receipt</p>
            </div>
            <div class="text-right">
                <p class="font-mono text-sm font-semibold text-slate-900 dark:text-white">{{ $payment->receipt_no }}</p>
        ...
        @if ($payment->status !== \App\Enums\ReceivedPaymentStatus::Cleared)
            {{-- A voided or refunded receipt still prints, and says so across the top: a copy that
        ...
        <dl class="mt-6 space-y-3 text-sm">
            ...
            <div class="flex justify-between"><dt class="text-slate-500">For</dt>
                <dd>{{ $payment->fee?->fee_type?->label() }}</dd></div>
        ...
        <p class="mt-8 text-xs text-slate-500 dark:text-slate-400">
            Received by {{ $payment->receivedBy?->name ?? $payment->received_by_name }} ·
            entered {{ app_datetime($payment->recorded_at) }}
        </p>
    </div>
@endsection
