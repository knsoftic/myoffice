...
@if ($cards->isNotEmpty())
    <x-site.section :anchor="data_get($section ?? null, 'anchor')" background="surface" :label="$heading">
        <div class="flex flex-col gap-6 md:flex-row md:items-end md:justify-between">
            ...
            @elseif ($indexUrl)
                <x-site.button data-fx="rise" data-fx-delay="1" label="See all work" :url="$indexUrl" style="outline" icon="arrow-right" class="shrink-0" />
            @endif
        </div>
        ...
        <ul role="list" class="mt-12 grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
            @foreach ($cards as $card)
                ...
                @endphp
                <li data-fx="tilt" data-fx-delay="{{ ($loop->index % 4) + 1 }}" class="group relative overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-card transition duration-200 hover:-translate-y-0.5 hover:shadow-card-hover focus-within:ring-2 focus-within:ring-brand-500/50 dark:border-white/10 dark:bg-slate-900">
