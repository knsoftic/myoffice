...
<footer class="dark" aria-labelledby="site-footer-heading">
    <div class="relative overflow-hidden bg-slate-950 text-slate-400">
        ...
        <div class="relative mx-auto max-w-screen-xl px-4 pb-10 pt-14 sm:px-6 lg:px-8 lg:pt-16">
            {{--
                `items-start`: grid rows stretch by default, so two short link columns beside a tall
                contact block were being held open to its height and the footer read as mostly empty
                space. Each column is now only as tall as what is in it.
            --}}
            <div class="grid items-start gap-12 lg:grid-cols-12 lg:gap-10">
                <div class="lg:col-span-4">
                    ...
                    {{--
                        A short row of real actions, built from the contact settings the block below
                        already prints as text.

                        The brand column held a logo, one line of tagline and then nothing — the
                        emptiest part of the page — while the one thing a visitor might want to do
                        from down here was three columns away and rendered as plain text. Repeating
                        the details is not the point; making them pressable is.

                        Each only appears when its setting is filled, so an installation that has
                        not entered a WhatsApp number does not get a dead button.
                    --}}
                    @php
                        $footerActions = array_values(array_filter([
                            $whatsappHref === null ? null : ['label' => 'WhatsApp', 'href' => $whatsappHref, 'icon' => 'chat-bubble-left-right', 'external' => true],
                            $phoneHref === null ? null : ['label' => 'Call us', 'href' => $phoneHref, 'icon' => 'phone', 'external' => false],
                            $email === '' ? null : ['label' => 'Email', 'href' => 'mailto:'.$email, 'icon' => 'envelope', 'external' => false],
                        ]));
                    @endphp
                            ...
                            @foreach ($footerActions as $action)
                                <li>
                                    <a
                                        ...
                                        class="inline-flex items-center gap-2 rounded-lg bg-white/5 px-3 py-2 text-xs font-medium text-slate-200 ring-1 ring-inset ring-white/10 transition hover:bg-white/10 hover:text-white focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-brand-400"
                                    >
                                        <x-ui.icon :name="$action['icon']" class="h-4 w-4 text-slate-400" />
                                        {{ $action['label'] }}
                                    </a>
                                </li>
                            @endforeach
            ...
            <div class="mt-12 flex flex-col gap-6 border-t border-white/10 pt-8 md:flex-row md:items-center md:justify-between md:pr-16">
