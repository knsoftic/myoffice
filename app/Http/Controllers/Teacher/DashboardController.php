...
/**
 * Teacher panel landing page (phase-01 §8).
 *
 * Phase 1 has no batch, timetable or attendance tables yet, so this screen shows **no numbers
 * it cannot prove**: only the signed-in teacher's own identity, account state and login
 * bookkeeping, plus empty states naming the phase that fills each section. No query here can
 * reach another user's row — batches, timetable, attendance and assignments arrive in phases
 * 16–20.
 *
 * Route: `teacher.dashboard` (`routes/teacher.php`, owned by the routes agent).
 */
final class DashboardController extends Controller
{
     ...
     */
    private const PORTAL = 'teacher_portal';
    ...
    {
        // Panel entry is already gated by `auth`, `active` and `panel:teacher`; this is the
    ...
    {
        $prefix = PanelType::Teacher->routePrefix();
