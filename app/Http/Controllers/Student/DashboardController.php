...
/**
 * Student panel landing page (phase-01 §8).
 *
 * Phase 1 has no institute tables yet, so this screen shows **no numbers it cannot prove**:
 * only the signed-in student's own identity, account state and login bookkeeping, plus empty
 * states naming the phase that fills each section. There is no query here that can reach
 * another student's row — course, batch, timetable, fees and results arrive in phases 14–21.
 *
 * Route: `student.dashboard` (`routes/student.php`, owned by the routes agent).
 */
final class DashboardController extends Controller
{
     ...
     */
    private const PORTAL = 'student_portal';
    ...
    {
        // Panel entry is already gated by `auth`, `active` and `panel:student`; this is the
    ...
    {
        $prefix = PanelType::Student->routePrefix();
