...
/**
 * Phase 15's rows in the four D60 manifests, checked against the thing they describe.
 *
 ...
 */
final class AdmissionManifestTest extends TestCase
{
    use InteractsWithRbac;
    use RefreshDatabase;

    /** The four public routes of phase-14-17 §7.10; every other Phase 15 route is under `admin.`. */
    private const PUBLIC_ROUTES = [
        'site.admission.create', 'site.admission.store', 'site.admission.submitted',
        'site.course-inquiries.store',
    ];

    /** The name prefixes this phase owns. */
    private const PREFIXES = [
        'admin.course-inquiries.', 'admin.student-applications.', 'admin.students.',
        'admin.admissions.', 'admin.demo-classes.', 'site.admission.', 'site.course-inquiries.',
    ];

    /** The six tables of phase-14-17 §2.11–§2.16. */
    private const TABLES = [
        'course_inquiries', 'course_inquiry_follow_ups', 'student_applications',
        'students', 'student_admissions', 'demo_classes',
    ];

    private const KINDS = ['index', 'show', 'form', 'board', 'calendar', 'wizard', 'print', 'public', 'export', 'dashboard', 'statement'];

    #[Test]
    public function every_phase_15_route_has_one_true_route_guard_row(): void
    {
        $rows = $this->rowsByRoute($this->manifest('route-guard-manifest'));
        $permissions = PermissionRegistry::permissionNames();
        $live = $this->phase15Routes();

        $this->assertNotEmpty($live, 'Phase 15 registers routes.');
        ...
        foreach ($live as $name => $route) {
            ...
            $this->assertSame(15, $row['owner_phase']);
        ...
        foreach ($rows as $name => $row) {
            if (($row['owner_phase'] ?? null) === 15) {
                $this->assertArrayHasKey($name, $live, sprintf('route-guard-manifest lists %s, which Phase 15 no longer registers.', $name));
            }
        }
    }

    #[Test]
    public function every_phase_15_get_route_has_a_screen_row(): void
    {
        ...
        foreach ($rows as $name => $row) {
            if (($row['owner_phase'] ?? null) === 15) {
                $this->assertArrayHasKey($name, $gets, sprintf('screen-manifest lists %s, which is not a Phase 15 GET route.', $name));
            }
        }
    }
    ...
    {
        $rows = collect($this->manifest('upload-manifest'))->where('owner_phase', 15);

        $this->assertCount(0, $rows);

        // A student photo is the one file §66 mentions, and it arrives as a `photo_path` chosen from
        // the media library rather than as an upload of this phase's own — so there is no route that
        // takes a file, and the manifest says so by having nothing to say.
        foreach ($this->phase15Routes() as $name => $route) {
    ...
    */
     ...
     */
    private function phase15Routes(): array
    {
        $routes = [];
