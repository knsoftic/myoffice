...
| INV-8 | A payment row is never edited. A wrong receipt is **voided** and re-entered; the two rows stay linked. | Policy `update` allows only `notes` / `reference_no` / `receipt_path` / **`invoice_id`** (the single concession, see below); FT-35 |
...
| 4 | Gated by **both** `project_payments.edit` **and** `invoices.edit` (D43's wording). **Unresolved, and deliberately not decided here:** §4.1 registers `project_payments` with **no `edit` ability** — "the same discipline on the software-house side" as `student_fee_payments`, where `edit` is withheld for ever (INV-8, INV-5). So either §4.1 gains an `edit` ability on a money module (a widening of the permission surface on a payment table, which this spine will not grant by itself), or D43's guard is re-expressed as an existing ability — the narrower and safer reading being `project_payments.change_status` (already registered, already the voiding ability) **and** `invoices.edit`. Until the owner rules, **the conservative reading binds**: the re-link requires `invoices.edit` plus the strictest project-payment ability the grantee holds, and no new ability is seeded. |
...
| `superseded_by_id` | FK self | nullable | `nullOnDelete` - forward pointer |
...
loser is recorded as a superseded row with its reason. `UNIQUE uq_cr_superseded_by(superseded_by_id)` - a
row has at most one successor. `INDEX (collaborator_id, referral_date)`, `(referral_code)`,
...
| `project_payments` | `Finance` | `banknotes` | `READ` + `create` + `print` + `export` + `STATUS` + `MONEY` + `LOGS` | the same discipline on the software-house side |
...
| `change(Model $subject, Collaborator $new, string $reason): CollaboratorReferral` | supersedes, never mutates: old row -> `superseded`, `effective_to = today`, `superseded_by_id`, `change_reason`, `changed_by`; the new row starts today; the **open entitlement is superseded**; **no existing ledger row is ever re-pointed** (INV-18); writes an audit row with old and new collaborator names (§37, §107) | `ReferralChanged` |
...
| `payoutsPaidTotal(Collaborator $c, ?DateRange $r = null): string` | "total paid out" as a published figure (F-4.8), derived from the §6.5.1 canonical SQL over **live allocations on `paid` payouts** (INV-23) — never from `PayoutStatus` alone and never `SUM(collaborator_payouts.amount)` in a caller. Phase 13's finance register and Phase 19-23's collaborator reports call this; a `SUM()` over payouts in a report class is a review failure (INV-26, FT-42) | - |
...
| | `commissionAccruedTotal(Collaborator $c, ?DateRange $r = null): string` - "total commission earned" as a published figure (F-4.8), derived from the §6.5.1 canonical SQL over `signed_amount` on earning entries net of reversals and clawbacks, so the figure a report prints is the figure the statement balances to. Nothing outside this class and `CollaboratorWalletService` may sum it (INV-26, FT-42) | - |
...
| FT-42 | `test_only_the_wallet_and_statement_services_compute_balances` | a static assertion that controllers, widgets, exports and notifications depend on `CollaboratorWalletService` / `CollaboratorStatementService` and contain no `SUM(` over the ledger |
...
| `DEVELOPMENT_LOG.md` §4: **D43** - the single concession to INV-8: `project_payments.invoice_id` may move NULL -> value -> NULL, by `InvoiceService` alone, reason-mandatory, audited, gated by `project_payments.edit` **and** `invoices.edit`, with zero commission effect (§1.4, F-4.10) | so nobody later "tightens" INV-8 back and breaks Phase 13's advance-to-invoice attachment, and nobody widens it to a money column |
