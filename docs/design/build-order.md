...
| 11 | Drift fixes (round 3) — the corrections this file took from `consistency-audit-round-3.md` |
...
| F17 | **13** — the **company-wide** (all-collaborator) payout and commission period totals for P&L block C | `CollaboratorWalletService::payoutsPaidTotal(?Collaborator $c, ?DateRange $r = null): string` and `CollaboratorStatementService::commissionAccruedTotal(?Collaborator $c, ?DateRange $r = null): string`, **`$c = null` meaning company-wide** | nothing: the methods exist in the same release as the services (Phase 12). Phase 13 never sums payouts or ledger rows itself (INV-26) — it calls the company-wide form: `payoutsPaidTotal(null, $range)` | **12** (spine §6.2, §6.5.1) | spine §6.2 / §6.5.1, F-4.8 + **ND-6** — **SATISFIED (RD-4)**: phase-13 §13.1 now reads SATISFIED (its round-2 "Still open" marker is gone), §6.7.3 calls `payoutsPaidTotal(null, $range)` and `commissionAccruedTotal(null, $range)` by name, and phase-10-12 §6.3 / §13.2 publish the same two signatures |
...
| **3** | INV-1…INV-16 · `cms:verify-published-snapshots` green · one `website` group, label "Website & Forms", sort **75**, **34** merged keys — 13 from phase-03 §5.1a + 21 from phase-04 §5, name-diffed (F-6.3, ND-9, **RD-6**; "35" was one high) · **no** public route carries `module:` or `can:`; `site_module` 404s (**D26**) · `RichText::sanitize()` on write **and** render (**D25**) · publish-by-snapshot + version-stamp invalidation (**D22**) · cites D19, D22, D23, D24, D25 · the four manifest files created (E4) |
...
**B-H2 is closed (RD-4):** the spine answered it with the nullable-`Collaborator` parameter and three
contracts implemented that form, so the row below is kept only as a record of the decision and its
reasoning — it is **not** an open question and the rejected `*All` sibling must not be built.
...
| B-H2 | **F17** — P&L block C needs **company-wide** payout and commission period totals, but the spine published only per-collaborator forms. Should the spine add a nullable-`Collaborator` parameter, or a sibling all-collaborator method? | **ANSWERED by the spine — the nullable parameter (ND-6, RD-4).** No sibling method exists: `payoutsPaidTotalAll()` / `commissionAccruedTotalAll()` appear in **no** contract and must not be introduced here. The published surface is `payoutsPaidTotal(?Collaborator $c, ?DateRange $r = null): string` (`CollaboratorWalletService`) and `commissionAccruedTotal(?Collaborator $c, ?DateRange $r = null): string` (`CollaboratorStatementService`), where **`$c = null` means company-wide** — spine §6.2, §6.5.1; implemented identically in phase-13 §6.7.3 / §13.1, phase-10-12 §6.3 / §13.2 and phase-13's test 55 / FT-42. **Why the nullable form is safe here, and safer than a sibling:** (1) parameter 1 has **no default**, so a caller must write `payoutsPaidTotal(null, $range)` — "every collaborator in the business" is always a typed word at the call site and can never be an *omitted* argument, which was the whole worry; (2) a null collaborator makes the `DateRange` **mandatory** and **both-null throws**, so a company-wide all-time figure cannot be produced by accident; (3) it is the **same canonical SQL of §6.5.1** with one predicate dropped — one query, never a PHP loop — so the company-wide figure is the sum of the per-collaborator figures **to the paisa** by construction, which two separately-maintained method bodies could not guarantee; (4) one name per figure keeps INV-26 enforceable: a reviewer greps two method names, not four, and a `SUM()` over payouts or allocations in a report class stays a review failure. Block C's memo is therefore **built**, not withheld. |
...
## 11. Drift fixes (round 3)

Applied from [`consistency-audit-round-3.md`](consistency-audit-round-3.md) §3 — item **RD-4**, plus the
consequential half of **RD-6** (this file carried the same stale key count). This file decides **sequence
only**; the RD-4 rows below removed a *design* this file had no authority to prescribe and which the spine
had already decided the other way. No slice, no dependency, no promoter and no phase verdict changed.

| RD id | Change made |
|---|---|
| **RD-4** | **§4 row F17 closed and re-pointed.** It read *"open; see §10"* and described the needed artefact as "a spine method taking a nullable `Collaborator` **or a sibling all-collaborator form**". It now records the artefact that exists — `CollaboratorWalletService::payoutsPaidTotal(?Collaborator $c, ?DateRange $r = null): string` and `CollaboratorStatementService::commissionAccruedTotal(?Collaborator $c, ?DateRange $r = null): string`, **`$c = null` = company-wide** — names **Phase 12** as the completer (the methods ship with the services, so nothing is deferred), states that Phase 13 calls `payoutsPaidTotal(null, $range)` rather than summing anything itself (INV-26), and cites spine §6.2 / §6.5.1 + F-4.8 + **ND-6** with the verdict **SATISFIED** — phase-13 §13.1 itself now reads SATISFIED and §6.7.3 calls both company-wide forms by name, so this file was the last place calling the item open. |
| **RD-4** | **§10.3 row B-H2 answered, and the rejected design struck out.** The row recommended *"a sibling method on each service (`payoutsPaidTotalAll(?DateRange)`, `commissionAccruedTotalAll(?DateRange)`) … a nullable parameter on a money total is easy to pass by accident"* — a design that appears in **no** contract, while the spine, phase-13 and phase-10-12 all implement the nullable first parameter. Because this file is the sequencing document a builder opens *before* the contracts, that row would have produced two method names for one figure. It now records the answer and **why the nullable form is safe here**: parameter 1 has **no default**, so `null` is always typed at the call site and never an omitted argument; a null collaborator makes the `DateRange` mandatory and **both-null throws**; it is the §6.5.1 canonical SQL with one predicate dropped — **one query, never a loop** — so the company-wide figure equals the sum of the per-collaborator figures **to the paisa**; and one name per figure keeps INV-26 greppable. A note under the §10.3 heading marks B-H2 **closed** so the row is not re-read as an open question, and says the `*All` sibling must not be built. |
| **RD-6** *(consequential)* | **§8's Phase 3 definition-of-done delta said "35 merged keys"** — the same count RD-6 corrects in `resolutions.md`. Changed to **34** (13 from phase-03 §5.1a + 21 from phase-04 §5, both contracts having recounted and name-diffed the keys), citing F-6.3 / ND-9 / RD-6 and noting that 35 was one high. A definition-of-done row is checked literally at the end of a phase, so a wrong total here would have failed Phase 3 or invited an invented 22nd Phase-4 key. |

**No money guarantee was weakened.** Both F17/B-H2 edits move this file *onto* the spine's published surface, which
is the one INV-26 protects: the two services remain the only code that may compute a balance, the company-wide
figure is derived by the same SQL as the per-collaborator one, and no caller is given permission to sum
payouts, allocations or ledger rows. RD-6 is a key **count**, not a key list: no setting was added, removed
or renamed. **Filed elsewhere:** RD-1, RD-3, RD-7 (phase contracts), RD-2 and RD-6's canonical half
(`resolutions.md` §2.4 / §3.6 F-6.3), RD-5, RD-8 (`data-model/04-finance-collaborator.md`).
