...
| `App\Support\Money` - **SATISFIED by phase-01 §3**, which now publishes the canonical surface of **21 methods**: `add`, `sub`, `mul`, `div`, `percentage($base,$rate)`, `percentageOf($part,$whole)`, `compare`, `isZero`, `isNegative`, `abs`, `min`, `max`, `sum(array)`, `round($v,$scale=2)`, `roundTo($amount,int $nearest)`, `prorate($amount,$part,$total)`, `distribute($amount,int $parts,RemainderPlacement $r = RemainderPlacement::First): array`, `allocate($amount, array $weights)`, `toMinor`, `fromMinor`, `format` - **bcmath only, intermediate scale 10, final half-up at 2, strings in and out, never a float** - plus `App\Enums\RemainderPlacement` (`first`, `last`, `largest`) | Phase 1 §3 (resolutions F-4.11) | FIN-12 tests this exact list directly, as the foundation of every money figure; the class change itself belongs to the Phase 1 remediation team |
