...
| 17 | `add_guard_unique_indexes_to_financial_tables` | the indexes that need file 16's columns: `uq_cr_student_current`, `uq_cr_project_current`, `uq_cr_client_current`, `uq_cr_lead_current`, `uq_ccs_open`, `uq_cce_current`, `idx_cpa_entry_active`, `uq_cpacc_default`. **`collaborator_referrals.superseded_by_id` carries `INDEX idx_cr_superseded_by` — a plain, NON-unique index (spine §2.8, ND-12)**, written with the ordinary indexes and **not** in this guard list: one winner may legitimately supersede several rows (`change()`'s predecessor plus every `recordLosingCandidate()` loser), so a unique index there would 1062 on a legal write. The "one active referral per subject" guarantee lives entirely in the four `uq_cr_*_current` indexes above |
...
| Guards | **[D-IMP-2]** `StudentFeePayment`, `ProjectPayment`, `PaymentReversal`: `creating` throws `DirectPaymentWriteException` unless `PaymentService::isWriting()`; `updating` throws when a dirty attribute is outside `{notes, reference_no, receipt_path, status, refunded_amount, commission_state, commission_skip_reason, commission_skip_detail, commission_attempts, commission_processed_at, collaborator_id, collaborator_referral_id, updated_by, updated_at}` — **plus `invoice_id` on `ProjectPayment` only** (D43, the single concession to INV-8; see the quoted clause under this table). `StudentFeePayment` has no `invoice_id` and gains nothing. Factories, seeders and a future historical import use the single greppable escape hatch `PaymentService::allowDirectWrites(fn () => ...)`; CI greps for it outside `database/` and `tests/` |
...
**`ProjectPayment.invoice_id` inside `[D-IMP-2]`'s `updating` whitelist — D43, quoted verbatim (ND-2).**
This model guard is what *enforces* spine INV-8, so the whitelist here and the spine's lifecycle whitelist
must be the same sentence, not two paraphrases that drift. The spine whitelists
`notes` / `reference_no` / `receipt_path` / **`invoice_id`** (spine §1.4 INV-8), and the conditions attached
to that fourth entry are reproduced below **word for word from spine §1.4**; an edit to one is an edit to
both:
...
What this means for the code Phase 10 ships: the `updating` guard **permits** a dirty `invoice_id` on
`ProjectPayment` instead of throwing — without this, `InvoiceService::applyPayment()`'s conditional
`UPDATE project_payments SET invoice_id = :invoice WHERE id = :payment AND invoice_id IS NULL`
(phase-13 §6.3 rules 4-5) would throw on the model Phase 10 ships and no advance could ever be attached to
an invoice. The guard enforces condition (1) positively, by refusing a dirty `invoice_id` unless the call is
inside `InvoiceService` (the same `isWriting()`-style flag pattern, asserted by test 41a); conditions (2)
and (3) are enforced by `InvoiceService` and its Form Request, condition (4) by the fact that the guard is
the only other writer of the column and reads no ledger row. **`PaymentService` itself never writes
`invoice_id` on an existing row** — it sets it once at insert (`recordProjectPayment()`, §6.3) and never
again. No other money column is added to the whitelist by this or any other concession; `amount`, `paid_on`,
`payment_method_id` and every commission column stay outside it for ever (INV-8).
...
| `project_payments` | `Finance` | `banknotes` | false | `READ` + `create` + `print` + `export` + `STATUS` + `MONEY` + `LOGS` + **`link_invoice`** | `edit`, `delete` (for ever - INV-8, INV-5); voiding is `change_status`. **`link_invoice` is not one of them**: it is the single narrow, non-preset addition **D43** needs (spine §4.1, ND-1) - see the block under this table |
...
**`project_payments.link_invoice` - registered here, narrow by construction (spine §4.1, ND-1 / RD-1).**
The `project_payments` row above **is** the build artefact for that slug's permission names, because Phase 10
is where the four spine slugs enter `PermissionRegistry`. Omitting `link_invoice` from it would mean the
permission is created by nobody, and phase-13 §7.1's `can:project_payments.link_invoice` apply / unapply
routes would be unreachable for **every** role - an unregistered name inside a `can:` middleware is not a
grant, it is a locked door, and `invoices.paid_amount` would stay permanently wrong for every advance.

| Property | Statement |
|---|---|
| What it permits | **Exactly one mutation: the D43 invoice-link field move** - `project_payments.invoice_id` travelling NULL -> value -> NULL, through `InvoiceService` alone, while the payment is not `voided`, with a mandatory reason, audited, and with zero commission effect (§2.3 `[D-IMP-2]`'s quoted whitelist clause; phase-13 §6.3 rules 4, 5 and 5a). **It permits no other mutation of any kind**: not `amount`, `paid_on`, `payment_method_id`, `reference_no`, `status`, `collaborator_id`, `collaborator_referral_id` or any commission column; not create, refund, void, approve or delete; and nothing whatsoever on `student_fee_payments`, `payment_reversals` or the ledger |
| Preset membership | **None, explicitly.** It belongs to none of Phase 1 §4's ten presets (`READ`, `CRUD`, `CRUD_FULL`, `APPROVE`, `STATUS`, `ASSIGN`, `FILES`, `MONEY`, `REPORTS`, `LOGS`) and specifically **not to `MONEY`**, so holding it reveals no amount by itself. It is written literally on this one slug, on no other slug, and no preset may ever absorb it |
| Who holds it | `RoleSeeder` grants it to the **Accountant** role only - the role that already holds `invoices.edit` and records project receipts - plus Admin / Super Admin through their blanket grants. **No other seeded role receives it**; widening it later is an explicit, audited role edit, never a preset change |
| Always paired | The route and the policy demand `project_payments.link_invoice` **and** `invoices.edit` together; holding either one alone is a **403** (phase-13 §7.1 and its test 53; spine FT-52) |
| Still no `edit` | `project_payments` keeps **no `edit` and no `delete`, for ever** (INV-8, INV-5), so "a received payment is never editable" stays literally true. A narrow ability must **never** be widened into a general `edit`: a registered `edit` on a money module is exactly what a later role edit, seeder or policy would quietly reuse for a real edit, while a single-purpose ability cannot be reused because no other code path checks it |
| Who declares what | The **`Ability` enum case** - `Ability::LinkInvoice` -> `link_invoice` - belongs to **Phase 1**, like every other ability name (spine §13.2; §13.1 below carries the ask). The **per-slug ability list** that puts it on `project_payments` and nowhere else belongs to **this phase**, §4.1. Both must land: an enum case with no registry row is not a grant, and a registry row naming a case that does not exist does not compile |
...
| `recordLosingCandidate(CollaboratorReferral $winner, Collaborator $loser, ReferralContext $ctx, string $reason): CollaboratorReferral` | the loser of a URL-versus-receptionist race, written **only here** because only the spine may write `collaborator_referrals` (F-4.4, spine §2.8): `status = superseded`, `superseded_by_id = $winner->id`, `effective_from = effective_to = today`, `commission_eligible = false`, `change_reason = $reason` (**mandatory**), `changed_by` = the actor, evidence from `$ctx`. **Several losers may point at one winner (ND-12):** `superseded_by_id` carries the plain, **non-unique** `idx_cr_superseded_by` (spine §2.8), so this method may be called more than once against the same winner, and may be called in the same transaction as `change()` — which also writes `superseded_by_id = winner` — without a 1062. Neither writer "owns" the column, and **no guarantee rests on it**: "one active referral per subject" is `uq_cr_*_current` over `current_guard` (INV-18), and a superseded row has `current_guard = NULL`. Phase 9's `ReferralAttributionResolver` (phase-08-09 §6.3 modifier 7) is the caller |
| `change(Model $subject, Collaborator $new, string $reason): CollaboratorReferral` | supersedes, never mutates: old row -> `superseded`, `effective_to = today`, `superseded_by_id`, `change_reason`, `changed_by`; the new row starts today; the open entitlement is superseded; **no existing ledger row is ever re-pointed** (INV-18); writes an audit row carrying old and new collaborator names (§37, §107). Its `superseded_by_id` write is **not** protected by a unique index and must not be read as one (ND-12): this method and `recordLosingCandidate()` may both point at the same winner; the uniqueness that matters is `uq_cr_*_current` |
...
| `payoutsPaidTotal(?Collaborator $c, ?DateRange $r = null): string` | total **paid out**, derived from the spine §6.5.1 canonical SQL over live (`is_released = 0`) `collaborator_payout_allocations` on `paid` payouts - never `SUM(collaborator_payouts.amount)` and never `status = paid` on the payout row (INV-23); the range filters the payout's business date `paid_on`. **`$c = null` means company-wide (ND-6, spine §6.2 / §6.5.1):** the same SQL with the `a.collaborator_id = :id` predicate dropped - **one query, no PHP loop over collaborators** - so the company-wide figure equals the sum of the per-collaborator figures to the paisa (FT-42). The first parameter has **no default** (a caller writes `payoutsPaidTotal(null, $range)`, so company-wide is never an omitted argument) and a null collaborator **requires** a `DateRange`; both null throws. Published so phase-13's P&L block C and phase-19-23's §99 reports can read a total **without summing money themselves** (F-4.8, INV-26) |
...
| Method | Guarantees |
|---|---|
| `commissionAccruedTotal(?Collaborator $c, ?DateRange $r = null): string` | total commission **accrued** (earnings, net of reversals and clawbacks) over the range, derived from the spine §6.5.1 canonical SQL on `signed_amount`, ranged on `transaction_date`. **`$c = null` means company-wide (ND-6):** the same SQL with the `e.collaborator_id = :id` predicate dropped, one query, exactly equal to the sum of the per-collaborator figures; same call-site discipline as `payoutsPaidTotal()` (no default on the first parameter, a `DateRange` mandatory when it is null). Published for the same reason as `payoutsPaidTotal()`: phase-13 §6.7.3's P&L memo and phase-19-23 §6.20's reports call it instead of writing a `SUM()` (F-4.8, INV-26) |
...
| 34a | `test_one_winner_may_supersede_several_losers` (spine FT-53, ND-12) | `superseded_by_id` is indexed **non-uniquely**: in one transaction `change()` supersedes the previously active referral pointing at the winner **and** two `recordLosingCandidate()` calls point at the same winner - three rows share the winner id, each with its own mandatory reason, and no 1062 is raised; `uq_cr_student_current` / `uq_cr_project_current` still refuse a second **active** referral for that subject; `information_schema` reports `idx_cr_superseded_by` as `NON_UNIQUE = 1` |
...
| 41a | `test_invoice_id_is_the_only_document_link_the_guard_permits` ([D-IMP-2], D43, ND-2) | `ProjectPayment::update(['invoice_id' => $id])` from outside `InvoiceService` throws (condition 1); the same update **inside** `InvoiceService::applyPayment()` succeeds, affects exactly one row, writes one `activity_log` row with old / new `invoice_id` and the mandatory reason, and leaves the collaborator's ledger and wallet **byte-identical** (condition 4, spine test FT-52 / phase-13 test 24); `unapplyPayment()` is the mirror; a `voided` payment is refused; a dirty `amount`, `paid_on`, `payment_method_id` or any commission column still throws in the `[D-IMP-2]` `updating` guard (and the policy refuses the request-level edit, spine FT-35), **including** when the call is inside `InvoiceService`; `StudentFeePayment` has no `invoice_id` at all; `PaymentService` never appears as the writer of an existing row's `invoice_id` in a static scan |
...
| 55 | `test_only_the_wallet_and_statement_services_compute_balances` (FT-42) | controllers, widgets, exports and notifications depend on `CollaboratorWalletService` / `CollaboratorStatementService` and contain no `SUM(` over the ledger. **Company-wide identity (ND-6):** with several collaborators in the fixture, `payoutsPaidTotal(null, $r)` equals the sum of the per-collaborator calls **to the paisa** and runs in **one** query (asserted by query count, not a loop); the same for `commissionAccruedTotal`; both throw when collaborator **and** range are null |
...
| # | Test name | Asserts |
|---|---|---|
| 61 | `test_authorization_matrix` (FT-46) | no `student_fee_payments.create` -> 403 on recording; no `collaborator_commissions.approve` -> 403 on approve **and** on `bulk-approve` with forged ids; no `.reject` -> 403 on `bulk-reject`; no `payment_reversals.create` -> 403 on refund; no `collaborator_payouts.approve` -> 403 on approve; no `wallet_reconciliation.change_status` -> 403 on run and recalculate; every 403 asserts **nothing was written**; and the registry this phase writes contains **`project_payments.link_invoice`** while containing **no `project_payments.edit`**, no `student_fee_payments.edit` and no `student_fee_payments.link_invoice`, with `link_invoice` present in **no** ability preset and granted to **Accountant** alone among seeded non-admin roles (§4.1, spine FT-52; the behavioural half is phase-13's test 53) |
...
| `App\Enums\Ability` gains **one** case, `LinkInvoice` -> `link_invoice` (spine §13.2, ND-1). The **enum case** is Phase 1's, like every ability name; the **per-slug list** that places it on the `project_payments` slug - and on no other slug and in no preset - is **§4.1 of this contract** (RD-1). the shared `Ability::Edit` case stays as it is and is **never listed on the `project_payments` or `student_fee_payments` slugs** | 1 | without the case the §4.1 row does not compile; without the §4.1 row the permission is never created and phase-13 §7.1's two D43 routes are unreachable for every role |
| `RoleSeeder`: grant the Collaborator role `collaborator_portal.wallet` + `.payouts`, the Student role `student_portal.fees` + `.payments`, the Client role `client_portal.payments`; and grant **`project_payments.link_invoice` to the Accountant role only** (no other seeded role below Admin - spine §13.2, ND-1) | 1 | otherwise every panel screen this contract builds is 403 on day one (Q-A), and the D43 invoice-link move is held by nobody |
...
| Phase 13 and Phase 23 read collaborator totals through `CollaboratorWalletService::payoutsPaidTotal()` and `CollaboratorStatementService::commissionAccruedTotal()` - **never a `SUM()` of their own, and never a loop over collaborators**. A **company-wide** period total (phase-13 §6.7.3's P&L block C and its commission memo) is the **same two methods called with a null collaborator**: `payoutsPaidTotal(null, $range)` / `commissionAccruedTotal(null, $range)` (ND-6) | 13, 23 | F-4.8, INV-26 - the company-wide aggregate belongs to the service that owns the canonical SQL, not to the caller |
...
| **Phase 13 is the only permitted user of `[D-IMP-2]`'s `invoice_id` whitelist entry (D43, ND-2)**, and only through `InvoiceService::applyPayment()` / `unapplyPayment()`: `InvoiceService` is the sole writer, the payment must not be `voided`, a reason is mandatory and audited, the act has zero commission effect, and the actor holds **`project_payments.link_invoice` + `invoices.edit`** (spine §1.4 guard 4, §4.1; phase-13 §6.3 rules 4-5 and 5a, §7.1). Phase 13 must not widen the whitelist, must not ask for `project_payments.edit` (it does not exist), and must not write `invoice_id` from a controller, job, seeder or factory | 13 | the guard this contract ships is what makes INV-8 true at runtime; the one concession stays exactly one column, one service and one pair of abilities |
...
| F-4.4 | §6.3: new published method `ReferralService::recordLosingCandidate($winner, $loser, $ctx, $reason): CollaboratorReferral` (status `superseded`, `superseded_by_id`, `commission_eligible = false`, reason mandatory) - the only write path for a losing candidate. **Amended by ND-12 below:** its row no longer claims `uq_cr_superseded_by` limits it to one loser per winner - that index is now a plain, non-unique `idx_cr_superseded_by`. |
| F-4.8 | §6.3: `CollaboratorWalletService::payoutsPaidTotal(Collaborator, ?DateRange): string` and `CollaboratorStatementService::commissionAccruedTotal(Collaborator, ?DateRange): string` added, both derived from the spine §6.5.1 canonical SQL; §13.2 names phases 13 and 23 as callers that never re-sum (INV-26 intact). **Superseded by ND-6 below:** both first parameters are now `?Collaborator` — a null means company-wide. |
...
[financial spine](../design/finance-commission-spine.md) remains authoritative on money: **no invariant was
relaxed, no duplicate-prevention layer removed and no money column gained a write path.** The one whitelist
entry added below is a **document link**, not money, and it is the single concession the spine itself grants
as D43.

| ND | Change made |
|---|---|
| ND-2 | **`[D-IMP-2]`'s `updating` whitelist now admits `invoice_id` — on `ProjectPayment` only.** Without it `InvoiceService::applyPayment()`'s conditional `UPDATE project_payments SET invoice_id = … WHERE id = … AND invoice_id IS NULL` would throw on the model this contract ships, so D43's concession (spine §1.4 INV-8) was unreachable and `invoices.paid_amount` could never be right for an advance. §2.3's Guards row adds the column; a new block under the table carries the spine's **canonical whitelist clause quoted verbatim** (InvoiceService only · reason mandatory · audited per §107 / INV-24 · zero commission effect · payment not `voided` · `project_payments.link_invoice` + `invoices.edit`) and states what the guard enforces itself versus what `InvoiceService` enforces, that `PaymentService` never rewrites `invoice_id` on an existing row, that `StudentFeePayment` has no such column, and that **no money column joins the whitelist**. §13.2 adds the behavioural contract binding Phase 13 to that one service and that one ability pair; new test **41a** asserts every clause, including that the ledger and wallet are byte-identical across an apply / unapply cycle. The quoted block exists in exactly two files and an edit to one is an edit to both. |
| ND-6 | **The two published totals gained their company-wide form.** §6.3: `payoutsPaidTotal(?Collaborator $c, ?DateRange $r = null): string` and `commissionAccruedTotal(?Collaborator $c, ?DateRange $r = null): string` — **a null collaborator means company-wide**, defined in the spine §6.5.1 as the same canonical SQL with the `collaborator_id` predicate dropped (A ranged on `transaction_date`, B on the payout's `paid_on`), **one query and never a PHP loop**, so the company-wide figure is exactly the sum of the per-collaborator figures. The first parameter has no default and a null collaborator requires a `DateRange`. §13.2's row tells Phase 13 and Phase 23 to call `…(null, $range)` for a company-wide period total instead of looping or summing; test **55** asserts the paisa-exact identity and the single query. INV-26 is untouched: the aggregate stays inside the two services that own the canonical SQL. |
| ND-12 | **`collaborator_referrals.superseded_by_id` is indexed, not unique.** §2.2 migration **17** no longer lists `uq_cr_superseded_by` among the guard unique indexes; the column carries the plain `INDEX idx_cr_superseded_by` (spine §2.8), because one winner legitimately supersedes several rows — `change()`'s predecessor **plus** every loser `recordLosingCandidate()` records — and a unique index would raise 1062 on a legal write and destroy attribution evidence. §6.3's `recordLosingCandidate()` row drops the claim that "`uq_cr_superseded_by` keeps it to one loser per winner" and states the opposite explicitly; §6.3's `change()` row says the same; both name **`uq_cr_*_current` over `current_guard`** as the sole carrier of the "one active referral per subject" guarantee (INV-18). New test **34a** asserts three rows may share one winner while a second active referral is still refused. |

---

## Drift fixes (round 3)

Closing the residual items of [`../design/consistency-audit-round-3.md`](../design/consistency-audit-round-3.md)
§3 that belong to this contract. The [financial spine](../design/finance-commission-spine.md) stays
authoritative on money: **no invariant was relaxed, no money column gained a write path, and no payment
table gained `edit` or `delete`.** The one ability registered below is narrower than the one D43 was
originally worded against, not wider.

| RD | Change made |
|---|---|
| RD-1 | **§4.1 now registers the ability the spine declares, so D43 exists at runtime.** The `project_payments` row reads `READ` + `create` + `print` + `export` + `STATUS` + `MONEY` + `LOGS` + **`link_invoice`** (spine §4.1, ND-1), and its "deliberately absent" cell still withholds `edit` and `delete` **for ever** (INV-8, INV-5) while stating that `link_invoice` is not one of them. A six-row property block under the table states that it permits **exactly one mutation - the D43 invoice-link field move** (`project_payments.invoice_id` NULL -> value -> NULL, `InvoiceService` only, not `voided`, reason mandatory, audited, zero commission effect) **and no other mutation of any kind**; that it is **explicitly in no preset and not in `MONEY`**; that `RoleSeeder` grants it to **Accountant only**; that it is always paired with `invoices.edit` (either alone is a 403); that a narrow ability must never be widened into a general `edit`; and - the ownership split RD-1 asked for - that the **`Ability::LinkInvoice` enum case is Phase 1's** while the **per-slug ability list for `project_payments` is this phase's**, because Phase 10 is where the four spine slugs are registered. §13.1 gains the matching blocking request for the enum case and extends the `RoleSeeder` row with the Accountant-only grant; test **61** now also asserts the registry contains `link_invoice`, contains **no** `project_payments.edit` / `student_fee_payments.edit` / `student_fee_payments.link_invoice`, holds `link_invoice` in no preset, and grants it to Accountant alone among seeded non-admin roles (spine FT-52; the behavioural half stays phase-13's test 53). Without this cell the permission was created by nobody and phase-13's two `can:project_payments.link_invoice` routes were unreachable for every role. |
