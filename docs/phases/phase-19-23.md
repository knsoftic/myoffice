...
| 3 | `layouts/site.blade.php`, `<x-site.seo>`, the **`site`** middleware alias (Phase 3 §6.10 ships `site`, `site.cache`, `site.preview`), **`App\Support\RichText::sanitize(string $html, string $profile = 'cms'): string`** - the one sanitiser, one class and one code path, with a **named profile allowlist** per call site (D25); §6.13 calls it as `RichText::sanitize($html, 'material')` - and `PublicCache` | required for the public verification page |
...
| INV-21-5 | A print template's `body_html` is **never** compiled as Blade, never passed to `eval`, `Blade::render`, `@php` or a view factory. It is sanitised by **`App\Support\RichText::sanitize($html, 'material')`** (Phase 3's single sanitiser over `mews/purifier`, called with the named `material` profile allowlist, D25) on save **and** again on render with the same profile, and rendered by a token replacer whose token list comes from `PrintTokenRegistry`. An unknown token renders empty. There is no second sanitiser class in the system and no second sanitising code path - the profile argument is an allowlist, not a second implementation. | `PrintTemplateService::render()` over `RichText::sanitize($html, 'material')`; PH21-25..PH21-29 |
...
| `sanitize(string $html): string` | **calls Phase 3's one sanitiser with a named profile: `App\Support\RichText::sanitize($html, 'material')`** (D25, audit F-2.5, ND-5). `RichText::sanitize(string $html, string $profile = 'cms')` is published by phase-03 §6.6 as **one class over `mews/purifier` with one code path and a per-call-site allowlist**; `material` is the committed profile for print templates and course/print material HTML - **not a second purifier profile and not a second sanitiser**. The `material` allowlist is the layout subset this phase needs (`div span p h1-h6 table thead tbody tr td th ul ol li img br hr strong em small b i sub sup` + `style`, `class`, `src` for `data:`/relative, `width`, `height`, `align`) and it still **strips**: `script`, `iframe`, `object`, `embed`, `link`, `meta`, `form`, `input`, every `on*` attribute, `javascript:` / `vbscript:` / external `url()`, every Blade construct (`{{`, `{!!`, `@php`, `@include`, `@extends`, `<?php`), with CSS sanitised for `@import`, `expression(` and external `url()`. This method therefore adds no filtering of its own - it only names the profile - and `render()` passes the stored HTML through `RichText::sanitize($html, 'material')` **again** before output, so a row written directly into the database cannot reach a print |
...
| **`App\Support\RichText::sanitize(string $html, string $profile = 'cms'): string`** with the committed **`material`** profile - **Phase 3 §6.6** (the owner; **one sanitiser class, one code path, a named allowlist per profile** - a profile is an argument, never a second purifier profile or a second class, D25) | §6.13's print templates need the wider layout allowlist (`div`, `h1`, `h5`, `h6`, `small`, `b`, `i`, `sub`, `sup`, `style`, `align`, `src` for `data:`) plus CSS sanitisation, or D52's snapshot fidelity and D53's token templates break. `PrintTemplateService::sanitize()` / `render()` call `sanitize($html, 'material')` and add no filtering of their own (ND-5) |
...
| Finding | Change made |
|---|---|
| F-2.5 | INV-21-5 and §6.13's `sanitize()` row now name `App\Support\RichText::sanitize()` (Phase 3, `mews/purifier`, D25) as the **only** sanitiser, applied on save **and** again on render; §1.2's Phase 3 row names it too. No `HtmlSanitizer`, no second purifier profile. *(Refined by **ND-5** in the next section: the call is `RichText::sanitize($html, 'material')` - the one class and one code path with a named profile allowlist. Still no second sanitiser and no second purifier profile; do not restore the "configured with this wider tag set" wording.)* |
...
| ND | Change made |
|---|---|
| ND-5 | §6.13's `PrintTemplateService::sanitize()` now **calls the one sanitiser with a named profile**: `App\Support\RichText::sanitize($html, 'material')` (phase-03 §6.6 publishes `sanitize(string $html, string $profile = 'cms')`; `material` is the committed profile whose allowlist is this phase's wider layout subset). The self-contradicting sentence - "configured with this wider tag set … It declares no `HtmlSanitizer` and no second purifier profile of its own" - is **removed**: a profile is an allowlist argument to **one class on one code path**, never a second purifier profile and never a second sanitiser. `render()` re-sanitises with the same profile. INV-21-5 and §1.2's Phase 3 row name the profiled call; §13.1 gains the explicit ask to **Phase 3 §6.6** for the profile parameter and the committed `material` profile (the audit noted neither contract recorded one); the F-2.5 convergence-log row carries a pointer so the old wording is not restored. D25, D52 and D53 are intact - the print template keeps `div` / `style` / `align` / `data:` images, so snapshot fidelity and token templates still hold. |
